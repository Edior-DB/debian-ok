...
if command -v brave-browser >/dev/null 2>&1; then
  ...
  if [ -d "$BRAVE_PROFILE_DIR" ]; then
    jq -s '.[0] * .[1]' "$BRAVE_PROFILE_DIR/Preferences" "$(dirname "$0")/brave-defaults.json" > "$BRAVE_PROFILE_DIR/Preferences.tmp" && \
...
if command -v firefox >/dev/null 2>&1; then
  ...
  if [ -n "$FIREFOX_PROFILE" ]; then
    ...
    jq -r 'to_entries[] | "user_pref(\"browser." + .key + "\", " + (if (.value|type=="string") then "\""+.value+"\"" else (.value|tostring) end) + ");"' "$(dirname "$0")/firefox-defaults.json" > "$FIREFOX_PROFILE/user.js"
